...
const logger = baseLogger.child({ scope: 'plugin:observability' })
...
export const ensureProjectRepository: StepCall<Project> = async (payload) => {
  const action = 'ensureProjectRepository'
  const gitlabProjectApi = payload.apis.gitlab as GitlabProjectApi
  try {
    logger.info({ action, repository: observabilityRepository }, 'Hook start')
    await gitlabProjectApi.getProjectId(observabilityRepository)
  } catch (e) {
    logger.warn({ action, repository: observabilityRepository, err: sanitizeCause(e) }, 'Repository not found, creating')
  ...
  logger.info({ action, repository: observabilityRepository }, 'Hook done')
  return okStatus
}

export const upsertProject: StepCall<Project> = async (payload) => {
  const action = 'upsertProject'
  try {
    if (specificallyDisabled(payload.config.observability?.enabled)) {
      logger.info({ action, projectId: payload.args.id, projectSlug: payload.args.slug }, 'Hook skipped')
      return okSkipped
    }
    ...
    logger.info({ action, projectId: project.id, projectSlug: project.slug }, 'Hook start')
    ...
    logger.info({ action, projectId: project.id, projectSlug: project.slug, result: yamlResult }, 'Gitlab config synced')

    await ensureKeycloakGroups(listPerms, keycloakApi)
    logger.info({ action, projectId: project.id, projectSlug: project.slug }, 'Keycloak groups synced')
    ...
    logger.error({ action, projectId: payload.args.id, projectSlug: payload.args.slug, err: sanitizeCause(error) }, 'Hook failed')
    return {
      status: {
        result: 'KO',
...
export const deleteProject: StepCall<Project> = async (payload) => {
  ...
  try {
    ...
    logger.info({ action, projectId: project.id, projectSlug: project.slug }, 'Hook done')
    return {
      status: {
        result: 'OK',
        message: 'Deleted',
