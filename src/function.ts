...
export const ensureProjectRepository: StepCall<Project> = async (payload) => {
  ...
  try {
    ...
    logger.warn({ action: 'ensureProjectRepository', repository: observabilityRepository, err: sanitizeCause(e) }, 'Repository not found, creating')
...
export const upsertProject: StepCall<Project> = async (payload) => {
  try {
    ...
    logger.error({ action: 'upsertProject', projectId: payload.args.id, projectSlug: payload.args.slug, err: sanitizeCause(error) }, 'Hook failed')
    return {
      status: {
        result: 'KO',
...
export const deleteProject: StepCall<Project> = async (payload) => {
  try {
    ...
    logger.error({ action: 'deleteProject', projectId: payload.args.id, projectSlug: payload.args.slug, err: sanitizeCause(error) }, 'Hook failed')
    return {
      status: {
        result: 'OK',
