...
export class ObservabilityRepoManager {
  ...
  private async getValuesFile(project: ProjectSchema): Promise<ObservabilityData | null> {
    try {
      ...
      logger.error({ action: 'getValuesFile', projectId: project.id, filePath: valuesPath, branch: valuesBranch, err: sanitizeCause(error) }, 'Failed to load values file')
      throw error
    }
  }
  ...
  public async commitAndPushYamlFile(project: ProjectSchema, filePath: string, branch: string, commitMessage: string, yamlString: string): Promise<void> {
    ...
    try {
      ...
      logger.error({ action: 'commitAndPushYamlFile', projectId: project.id, filePath, branch, err: sanitizeCause(error) }, 'Failed to commit values file')
      throw error
    }
  }
